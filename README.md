...
veilchen is a set of reusable Svelte 5 components for mobile apps, styled with
DaisyUI.
...
The following dependencies are included with veilchen and will be automatically
available:

- [Chart.js](https://www.chartjs.org/) - Powers the chart components (LineChart,
  BarChart, PolarAreaChart)
- [svelte-gestures](https://github.com/Rezi/svelte-gestures) - Enables swipe
  functionality in the Stack component
...
All dependencies including Chart.js and svelte-gestures are bundled with
veilchen, so no additional installation is required.
...
This is required because Tailwind's JIT compiler does not scan the
`node_modules` folder when excluded in `.gitignore`.

### Custom Theme

veilchen includes a custom DaisyUI theme inspired by violet flowers. The theme
features:
...
The theme is defined using modern OKLCH color space for perceptually uniform
colors and better accessibility. All colors meet WCAG contrast requirements.
...
Then activate the theme by adding `data-theme="veilchen"` to your `<html>` tag
in `src/app.html`:
...
- **AlertBox** - Alert messages with different types (info, error, warning,
  success)
...
```svelte
<script lang="ts">
  import { type OptionCardData, OptionCards } from "@thwbh/veilchen";
...
Transient notification surface with optional action button, designed for the
"undo" pattern.

**Props:**

- `position` - Vertical position ('top' or 'bottom', default: 'bottom')
- `align` - Horizontal alignment ('start', 'center', or 'end', default:
  'center')
- `class` - Optional CSS class
...
- `onDismiss` - Callback when snackbar is dismissed with reason ('timeout',
  'action', 'manual')
...
```svelte
<script lang="ts">
  import { snackbar, SnackbarContainer } from "@thwbh/veilchen";
...
- **Snackbar** - Transient notification with optional action button (undo
  pattern)
...
All components accept a `class` prop for custom styling. Since `class` is a
reserved keyword in JavaScript, it is internally mapped to avoid conflicts:
...
- **Stack**: Keyboard navigation with Arrow Left/Right, `role="region"`,
  `aria-live="polite"`
- **Stepper**: Keyboard navigation with Arrow keys (Left/Up for previous,
  Right/Down for next), `role="navigation"`
...
[GitHub](https://github.com/thwbh/veilchen).
