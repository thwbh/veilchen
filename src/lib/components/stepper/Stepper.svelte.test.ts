...
describe("Stepper", () => {
  ...
  test("calls onnext when next button is clicked", async () => {
    ...
    const nextButton = Array.from(container.querySelectorAll("button")).find((
      btn,
    ) => btn.textContent?.includes("Next Step")) as HTMLButtonElement;
  ...
  test("calls onback when back button is clicked from step 2", async () => {
    ...
    const backButton = Array.from(container.querySelectorAll("button")).find((
      btn,
    ) => btn.textContent?.includes("Previous")) as HTMLButtonElement;
  ...
  test("calls onfinish when finish button is clicked", async () => {
    ...
    const finishButton = Array.from(container.querySelectorAll("button")).find((
      btn,
    ) => btn.textContent?.includes("Finish")) as HTMLButtonElement;
  ...
  describe("Stepper hooks", () => {
    ...
    test("onbeforenext returning false blocks forward navigation", async () => {
      ...
      const nextButton = Array.from(container.querySelectorAll("button")).find((
        btn,
      ) => btn.textContent?.includes("Next")) as HTMLButtonElement;
      await fireEvent.click(nextButton);
