<script lang="ts">
  ...
  let steps = $derived(
    Object.values(restProps).filter((f): f is StepSnippet =>
      typeof f === "function"
    ),
  );
...
>
  <div
    class="stepper-content-wrapper {stickyButtons ? 'stepper-content-wrapper-sticky' : ''}"
  >
            ...
            >
              <div class="timeline-middle">
                {#if currentStep === step + 1}
                  <span class="badge badge-step {activeClass}">{stepLabel} {
                      step + 1
                    }</span>
                {:else}
    ...
    {#if !stickyButtons}
      <div class="stepper-buttons-wrapper">
        <div class="join grid grid-cols-2">
          <button
            onclick={() => move(-1)}
            class="join-item btn flex items-center justify-start"
          >
            <span>
              {#if leftCaret}
                {@render leftCaret()}
