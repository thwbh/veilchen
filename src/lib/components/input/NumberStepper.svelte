<script lang="ts">
   ...
   */
  interface Props
    extends Omit<HTMLInputAttributes, "value" | "type" | "children"> {
  ...
  // svelte-ignore state_referenced_locally
  const sharedSteps = useSingleStep
    ? (showLeftWheel ? decrementSteps : incrementSteps)
    : [];
  // svelte-ignore state_referenced_locally
  const sharedInitialStep = useSingleStep
    ? showLeftWheel ? initialDecrementStep : initialIncrementStep
    : 0;
  ...
  let activeIncrementStep = $derived(
    useSingleStep ? sharedStep : incrementStep,
  );
  let activeDecrementStep = $derived(
    useSingleStep ? sharedStep : decrementStep,
  );
  ...
  <div class="number-stepper-container">
    ...
    <div class="stepper-section">
      {#if showLeftWheel}
        ...
        {:else}
          <InlineNumberWheel
            numbers={decrementSteps}
            bind:value={decrementStep}
          />
        {/if}
      {/if}
      <button
    ...
    <div class="stepper-section">
      ...
      {#if showRightWheel}
        ...
        {:else}
          <InlineNumberWheel
            numbers={incrementSteps}
            bind:value={incrementStep}
          />
        {/if}
      {/if}
    </div>
