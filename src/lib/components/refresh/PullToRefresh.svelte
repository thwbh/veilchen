...
<div
  ...
  style="overflow-y: auto; overflow-x: hidden; -webkit-overflow-scrolling: touch; touch-action: pan-y; position: relative"
>
  <!-- Refresh indicator -->
  <div
    class="refresh-indicator"
    style="position: absolute; top: 0; left: 0; right: 0; height: {maxPullDistance}px; display: flex; align-items: center; justify-content: center; transform: translateY({(refreshing || isReleasing ? threshold : pullDistance) - maxPullDistance}px); transition: {isPulling ? "none" : "transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)"}; pointer-events: none; z-index: 10"
  >
    {#if indicator}
    ...
    {:else}
      <div
        ...
        style="width: 40px; height: 40px; border-radius: 50%; background: var(--color-primary, hsl(var(--p))); display: flex; align-items: center; justify-content: center; opacity: {pullProgress}; transform: rotate({pullProgress * 360}deg); transition: {isPulling ? "none" : "opacity 0.2s, transform 0.3s"}"
      >
        <svg
  ...
  <div
    ...
    style="transform: translateY({refreshing || isReleasing ? threshold : pullDistance}px); transition: {isPulling ? "none" : "transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)"}; padding-bottom: calc({bottomPadding}rem + env(safe-area-inset-bottom))"
  >
    {@render children()}
  </div>
</div>
