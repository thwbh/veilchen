<script lang="ts">
  ...
  let swipeStates = $state<
    Map<string, { translateX: number; isDragging: boolean }>
  >(new Map());
  ...
  <div class="toast {positionClass} {alignClass} {className} z-50">
    {#each snackbar.all as snackbarItem (snackbarItem.id)}
      ...
      <div
        ...
        style="transform: translateX({state.translateX}px); transition: {state.isDragging ? "none" : "transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)"}; cursor: {state.isDragging ? "grabbing" : "grab"}; touch-action: pan-y; user-select: none; opacity: {Math.max(0.3, 1 - Math.abs(state.translateX) / (swipeThreshold * 1.5))}"
