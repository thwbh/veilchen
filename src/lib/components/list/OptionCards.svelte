...
<div class="flex flex-col gap-3" {...props}>
  ...
  >
    {#each data as option}
      ...
      <button
        class="
          card bg-base-100 border-2 shadow-md transition-all hover:shadow-lg {isSelected
          ? 'border-primary'
            ...
            <div class="flex min-w-0 flex-1 items-center gap-2">
              ...
              <div class="min-w-0 flex-1">
                <div class="mb-1 flex items-center gap-2">
                  <h3 class="text-base-content text-base font-bold sm:text-lg">
                    {option.header}
                  </h3>
                  {#if option.highlight}
                    <span
                ...
                {#if option.badge}
                  <span
                    class="badge {option.badge.color || BadgeColor.Neutral} badge-sm text-xs"
                  >
                    {option.badge.text}
                  </span>
                {/if}
            ...
            >
              {#each option.metrics as metric}
                <div class="bg-base-200 rounded-lg p-2.5 sm:p-2">
                  <p class="text-base-content mb-0.5 text-xs opacity-60">
                    {metric.label}
                  </p>
                  <p class="text-base-content text-sm font-bold sm:text-base">
                    {metric.value}
                  </p>
                </div>
              {/each}
