...
>
  ...
  {#if leftAction}
    <div
      ...
      style="position: absolute; top: 0; left: 0; bottom: 0; width: 100%; display: flex; align-items: center; justify-content: flex-start; padding-left: 1rem; overflow: hidden; pointer-events: none"
    >
      {@render leftAction()}
    </div>
  {/if}
  ...
  {#if rightAction}
    <div
      ...
      style="position: absolute; top: 0; right: 0; bottom: 0; width: 100%; display: flex; align-items: center; justify-content: flex-end; padding-right: 1rem; overflow: hidden; pointer-events: none"
    >
      {@render rightAction()}
    </div>
  {/if}
  ...
  <div
    ...
    role="button"
    tabindex="0"
    style="position: relative; transform: translateX({translateX}px); transition: {isDragging ? "none" : "transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)"}; cursor: {disabled ? "default" : isDragging ? "grabbing" : isExecuting ? "wait" : "grab"}; background: var(--color-base-100, white); z-index: 1; touch-action: pan-y"
  >
    {@render children()}
  </div>
</div>
