<script lang="ts">
  ...
  let { value = $bindable(), data, header = undefined, onchange }: Props =
    $props();
    ...
    <div class="flex flex-col">
      {#each data as dataValue}
        <li>
          <button
            class="
              list-row flex w-full flex-col gap-2 text-left {value === dataValue.value
              ? 'bg-base-300'
              : ''}
            "
          ...
          >
            ...
            >
              ...
              {#if dataValue.label}
                {@const className = dataValue.value === value
