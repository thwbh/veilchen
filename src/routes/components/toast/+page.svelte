...
<div class="flex flex-col gap-6 p-4">
  ...
  <div class="alert alert-info">
    ...
    <span><strong>Try it:</strong> Swipe toasts left or right to dismiss
      them!</span>
  </div>
  ...
  <div>
    ...
    <p class="mb-2 text-sm opacity-70">
      When multiple toasts are triggered, they stack vertically. As toasts
      dismiss, others move up.
    </p>
    <button class="btn btn-primary" onclick={showMultiple}>
  ...
  <div>
    ...
    <div class="flex gap-3">
      <button class="btn btn-secondary" onclick={showLongDuration}>
        Long Duration (10s)
      </button>
      <button class="btn btn-accent" onclick={showPersistent}>
  ...
  <div class="bg-base-200 mt-4 rounded-lg p-4">
    ...
    <div class="text-sm">
      ...
      <pre class="bg-base-300 mb-2 rounded p-2"><code
				...
				></pre>
      <pre
        class="bg-base-300 mb-2 rounded p-2"
      ><code>toast.success('Success message');</code></pre>
      <pre
        class="bg-base-300 mb-2 rounded p-2"
      ><code>toast.error('Error message');</code></pre>
      <pre
        class="bg-base-300 mb-2 rounded p-2"
      ><code>toast.warning('Warning message');</code></pre>
      <pre
        class="bg-base-300 rounded p-2"
      ><code>toast.info('Info message');</code></pre>
    </div>
  </div>
</div>
